...
class ReceiptRepository extends CoreRepository
{
        ...
        return $models
            ...
            ->with([
                ...
                'instruction' => fn($q) => $q->select('locale', 'title', 'receipt_id')
                    ->where('locale', $this->language)
                    ->orWhere('locale', $locale),
                'nutritions.translation' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
    ...
    public function show(Receipt $model): Receipt
    {
        ...
        return $model->loadMissing([
            ...
            'ingredient' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'instruction' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'nutritions.translation' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'translations',
            'ingredients',
            'instructions',
    ...
    public function restShow(Receipt $model): Receipt
    {
        ...
        return $model->loadMissing([
            ...
            'category.translation' => fn($q) => $q->select('locale', 'title', 'category_id')
                ->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'stocks.countable',
            'stocks.countable.translation' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
        ]);
    }
