...
import { Divider, Input, Layout, Menu, Modal, Space } from 'antd';
...
const Sidebar = () => {
  ...
  const EXCLUDE_LIST = [
    ...
    'catalog.career',
    'blogs',
    // 'wallets', // Xuất hiện trên admin
    'wallet', // Xuất hiện trên seller
    'ads',
    'shop.ads',
    'marketing',
    'cashback',
    ...
    'payment.to.sellers',
    'completed.payments',
    'payment.to.deliverymans',
    'withdraws.seller_uncomplete',
    'withdraws.completed_seller_uncomplete',
    'withdraw.deliverymans',
    'withdraw.deliverymans_uncomplete',
  ...
  function filterMenu(menuList, excludeList) {
    return menuList
      .filter((item) => !excludeList.includes(item.name)) // Loại bỏ menu
      .map((item) => ({
        ...item,
        submenu: item.submenu
          ? filterMenu(item.submenu, excludeList) // Lọc submenu
          : undefined,
        children: item.children
          ? filterMenu(item.children, excludeList) // Lọc children menu
          : undefined,
      }))
      .filter(
        (item) =>
  ...
  // Dữ liệu menu ban đầu (menuList)
  const filteredMenuList = filterMenu(menuList, EXCLUDE_LIST);

  filteredMenuList.forEach((item) => {
    console.log(`📌 Menu: ${item.name} (${item.id})`);
    if (item.submenu) {
      item.submenu.forEach((sub) => {
        console.log(`   🔹 Submenu: ${sub.name} (${sub.id})`);
        if (sub.children) {
          sub.children.forEach((child) => {
            console.log(`      ⚡ Child Menu: ${child.name} (${child.id})`);
          });
        }
      });
    }
        ...
        >
          <Menu
            ...
            defaultOpenKeys={
              !navCollapsed
                ? data?.flatMap(
                    (menu) => (menu.submenu ? [menu.id] : []), // Chỉ mở submenu
                  )
                : []
            }
          >
