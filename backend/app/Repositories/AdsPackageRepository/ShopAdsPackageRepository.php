...
class ShopAdsPackageRepository extends CoreRepository
{
    ...
    public function show(ShopAdsPackage $model): ShopAdsPackage
    {
        ...
        return $model->loadMissing([
            'banner.translation' => fn($query) => $query->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'banner.shops:id',
            'banner.shops.translation' => fn($query) => $query
                ->select(['id', 'shop_id', 'locale', 'title'])
                ->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'adsPackage.translation' => fn($query) => $query->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'shop:id,logo_img',
            'shop.translation' => fn($query) => $query
                ->select(['id', 'shop_id', 'locale', 'title'])
                ->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
