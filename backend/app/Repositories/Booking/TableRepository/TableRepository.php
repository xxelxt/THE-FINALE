...
class TableRepository extends CoreRepository
{
    ...
    public function paginate(array $filter = []): LengthAwarePaginator
    {
        ...
        return $models
            ...
            ->with([
                ...
                'shopSection.translation' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            ])
    ...
    public function show(Table $model): Table|null
    {
        ...
        return $model->loadMissing([
            'shopSection.translation' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
            'shopSection.shop.translation' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale))
        ]);
    }
