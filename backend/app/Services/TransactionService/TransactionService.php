...
class TransactionService extends CoreService
{
    ...
    public function orderTransaction(int $id, array $data, $class = Order::class): array
    {
        ...
        $payment = $this->checkPayment(data_get($data, 'payment_sys_id'), $order, true, $data);
        ...
        if (isset($data['tips'])) {
            ...
            if ($transactionId && (int)$order->transaction?->payment_sys_id !== (int)data_get($data, 'payment_sys_id')) {
                $order->createManyTransaction($order->transaction->toArray());
                $totalPrice -= $order->transaction?->price;
            } else if ($order->transaction && $order->transaction->price < $totalPrice) {
                $order->createManyTransaction($order->transaction->toArray());
                $totalPrice -= $order->transaction?->price;
            }
